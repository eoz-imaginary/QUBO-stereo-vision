...
def kolmogorov_graph(image1, image2, K=None):
            ...
            if E < oldE:
                ...
                for r in range(dispL.shape[0]):
                    for c in range(dispL.shape[1]):
                        ...
                        if varsOnodes[i] != -1 and g.get_segment(varsOnodes[i]) == 0 and varsAnodes[
                            i] != -1 and g.get_segment(varsAnodes[i]) == 1:
                            print("DISASTER")
...
'''
OLD CODE
Code sources & inspiration from: 
https://github.com/roflmaostc/Fast-Approximate-Energy-Minimization-via-Graph-Cuts
https://gist.github.com/CrisDamian/2fa3fdd0074d3c7e3f98e02c124eda69#file-aexpansion_example-py

def Vpq(label1, label2, lamb):
    return abs(label1-label2)*lamb

def Dp(label, img_label):
    #label: new label for left image
    #image: right image
    return (abs(label**2 - img_label**2))**0.5

def get_neighbors(img, x, y):
    #returns the 4 neighboring pixel intensities of a given pixel
    if x >= len(img[0]) or x < 0 or y >= len(img) or y < 0:
        raise ValueError('Invalid x/y value')
    neighbors = []
    for a, b in zip([1, 0, -1, 0], [0, 1, 0, -1]):
        if (x + a < len(img[0]) and x + a >= 0) and (y + b < len(img) and y + b >= 0):
            neighbors.append(img[y + b][x + a])
    return neighbors
...
def init_graph(left, right):
    h, w = left.shape
    g = mf.Graph[int](h, w)
    nodes = g.add_grid_nodes((h, w))
    L = 16 #possible disparity labels

    #set up arbitrary labeling
    labels = np.zeros(left.shape, dtype=int)
    for i in range(0, w):
        for j in range(0, h):
            labels[i, j] = np.random.randint(0, L)

    #step = np.floor(255/L)
    #label_img = step*labels
    #cv2.imwrite('label_img.png', label_img)

    lamb = 2 #lambda constant weight for all n-links?

    #add n-link edges
    for x in range(0, w):
        for y in range(0, h):
            #iter through neighbors in order [top, left, bottom, right]
            for x_off, y_off in zip([1, 0, -1, 0], [0, 1, 0, -1]):
                if (0 <= x + x_off < w) and (0 <= y + y_off < h):
                    #print("X:", x, x+x_off)
                    #print("Y", y, y+y_off)
                    #MAY HAVE ISSUE W/ NODEIDS HERE
                    n_weight = Vpq(left[x, y], left[x+x_off, y+y_off], lamb)
                    g.add_edge(nodes[x, y], nodes[x+x_off, y+y_off], n_weight, 0)

    #add t-link edges
    for x in range(0, w):
        for y in range(0, h):
            neighbors = get_neighbors(left, x, y)
            #sum weight for all neighbors of pixel, then add Dp
            neighbor_weights = sum([Vpq(labels[x, y], node, lamb) for node in neighbors])
            t_weight = neighbor_weights + Dp(labels[x, y], right[x, y])
            t_weight_rev = neighbor_weights + (1-Dp(labels[x, y], right[x, y]))
            g.add_tedge(nodes[x,y], t_weight, t_weight_rev)
